...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    ...
    case 31010: // MAV_CMD_USER_1 (Test Trigger)
        // Story 1.2: 收到測試指令，發送 Start (31020) 给 AI 组件 (211)
        plane.ailanding.send_start_ai_landing(1, 10.0f);
        return MAV_RESULT_ACCEPTED;
...
void GCS_MAVLINK_Plane::handle_message(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_COMMAND_ACK: {
        ...
        if (AP::ailanding()) {
             AP::ailanding()->handle_command_ack(packet);
        }
        // Forward to common handler to avoid breaking other ACKs
        GCS_MAVLINK::handle_message(msg);
        break;
    }

    case 52100: // AI_LANDING_CORRECTION
        if (AP::ailanding()) {
             AP::ailanding()->handle_msg(msg);
        }
        break;

    case 52102: // AI_LANDING_STATUS
        if (AP::ailanding()) {
             AP::ailanding()->handle_status_msg(msg);
        }
        break;
